public class TraversalReview {
    ...
    public static boolean isIncreasing(TreeNode node) {
        ...
        if(ret) ret = isIncreasing(node.left);
        if(ret) ret = isIncreasing(node.right);
        return ret;
    } 
    ...
    public static boolean noSingleChildren(TreeNode node) {
        ...
        if(node.left == null && node.right != null) check = false;
        if(node.left != null && node.right == null) check = false;
        if(check) check = noSingleChildren(node.left);
        if(check) check = noSingleChildren(node.right);
        return check;
    }
    ...
    public static boolean hasZero(TreeNode node) {
        ...
        if(node.data == 0) zero = true;
        if(!zero) zero = hasZero(node.left);
        if(!zero) zero = hasZero(node.right);
        return zero;
    }
    ...
    public static boolean hasNonDivisible(TreeNode node, int k) {
        ...
        if(node.data % k != 0) ret = true;
        if(!ret) ret = hasNonDivisible(node.left, k);
        if(!ret) ret = hasNonDivisible(node.right, k);
        return ret;
    }
    ...
    public static String concatenate(TreeNode node) {
        String ret = "";
        if(node == null) return ret;
        ret += concatenate(node.left);
        ret += node.data;
        ret += concatenate(node.right);
        return ret;
    }
}